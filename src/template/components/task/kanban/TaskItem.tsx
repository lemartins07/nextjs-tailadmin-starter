...
type DraggableTask = Task & { index: number };
...
const TaskItem: React.FC<TaskItemProps> = ({
  ...
  const setDragDropRef = (node: HTMLDivElement | null) => {
    ref.current = node;
    drag(node);
    drop(node);
  };
  ...
  return (
    <div
      ref={setDragDropRef}
      style={{ opacity }}
      className="task relative rounded-xl border border-gray-200 bg-white p-5 shadow-theme-sm dark:border-gray-800 dark:bg-white/5"
      data-handler-id={handlerId}
    >
      <div className="space-y-4">
        <div>
          <h4 className="mr-10 mb-5 text-base text-gray-800 dark:text-white/90">
            {task.title}
          </h4>
          {task.projectDesc && (
      ...
      <div className="top absolute top-5 right-5 h-6 w-full max-w-6 overflow-hidden rounded-full border-[0.5px] border-gray-200 dark:border-gray-800">
